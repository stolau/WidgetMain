...
function createButton(nameBtn, idBtn, classBtn, servicePath, service)
{
	...
	if(classBtn === "buttonSearch") {
		...
		btn.onclick =  async function(){
			...
			for(var i = 0; i < dd.length; i ++)
			{
				...
				// Creates Initial Graph Window for data from Siptronix
					...
					var initDate = dd[i]["dateIssued"]["value"];
					var dateFrom = initDate.slice(0, 11) + "00:00:00Z";
					var dateTo = initDate.slice(0, 11) + "23:59Z";
					var idsBtnList = ["/type/3PhaseACMeasurement", "/id/Ritaharju_POS39_lighting", "/attributes/", "?lastN=100&dateFrom=" + dateFrom + "&dateTo=" + dateTo];;
		...
		btn.onclick = async function() {
			if(!onOffActivate) {
				var graphsList = [];
				var sipSearchList = ["activePower", "apparentPower", "current", "frequency", "powerFactor", "reactivePower", "totalActiveEnergyImport", "totalApparentPower", "totalReactivePower", "voltage"];
				for(var c = 0; c < sipSearchList.length; c++) {
					url = "https://cors-anywhere.herokuapp.com/pan0107.panoulu.net:8000/comet/STH/v1/contextEntities" + idBtn[0] + idBtn[1] + idBtn[2] + sipSearchList[c] + idBtn[3];
					var headers = await getHeader(servicePath, service);
					var dd = await browser(url, headers);
					var receivedData = dd["contextResponses"][0]["contextElement"]["attributes"][0]["values"];
